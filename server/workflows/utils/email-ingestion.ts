 ...
 *
 * Each sub-step is wrapped in `step.run()` for Vercel Workflow observability.
 *
...
export async function ingestEmail(
  ...
  if (!actualOrgId) {
    throw new FatalError("No organization found for user");
  }
  ...
  const incomingEmail = await step.run("fetch-incoming-email", async () => {
    ...
    if (actualOrgId && data.organization_id && data.organization_id !== actualOrgId) {
      throw new FatalError("Unauthorized: Email does not belong to organization");
    }
