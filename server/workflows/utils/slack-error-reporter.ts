 ...
 *
 * When an Vercel Workflow retry eventually succeeds, call markErrorsResolved(runId)
...
export interface ErrorEventParams {
  ...
  // Where (Vercel Workflow)
  workflowRunId?: string | null;
  workflowFunctionId?: string | null;
  attempt?: number | null;
...
export async function writeErrorEvent(params: ErrorEventParams): Promise<string | null> {
  try {
    ...
    const row = {
      ...
      workflow_function_id: params.workflowFunctionId ?? null,
...
/**
 * Mark all unresolved error rows for a given Vercel Workflow run as resolved.
 ...
 */
export async function markErrorsResolved(workflowRunId: string | null | undefined): Promise<void> {
...
/**
 * Build a short, user-safe request ID from an Vercel Workflow run ID.
 * Used in Tier B apology messages so users can reference an incident.
 */
export function toRequestId(workflowRunId: string | null | undefined): string {
  if (!workflowRunId) return "unknown";
  // Vercel Workflow run IDs are long ULIDs; take the last 8 chars for a terse reference
  return workflowRunId.slice(-8);
}
