/**
 * Email Approval Workflow - Vercel Workflow Function
 *
...
export const emailApprovalWorkflow = workflow.createFunction(
  {
...
Return JSON only:
{
  ...
  "body": "email body text"
    ...
    if (!UUID_RE.test(approvalRequest.id)) {
      throw new FatalError(`Invalid approvalRequest.id format: ${approvalRequest.id}`);
    }
    ...
    if (approval.data.action === "approve") {
      // Send email via Vercel Workflow send-email function
