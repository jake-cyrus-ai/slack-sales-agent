/**
 * Sync Calendar Events Vercel Workflow Function
 * 
 * Syncs Google Calendar events for a user and stores them in the database.
 * Migrated from Supabase Edge Functions to Vercel Workflow.
 */
...
export const syncCalendarEvents = workflow.createFunction(
  { 
  ...
  async ({ event, step }) => {
    ...
    try {
      ...
      // 5xx, network blips) should throw and let Vercel Workflow retry.
