 ...
 *
 * Two Vercel Workflow functions:
 *
 *   - `evaluateOrgReadiness` — runs the rules engine for ONE org.
 *     Fired by:
 *       • `workflow.send("org/readiness.evaluate", { orgId })` from the
...
// ---------------------------------------------------------------------------
export const evaluateOrgReadiness = workflow.createFunction(
  {
    ...
    // the Vercel Workflow plan limit and blocking the entire registration sync.
    retries: 2,
  },
...
export const evaluateAllOrgsReadiness = workflow.createFunction(
  {
  ...
  async ({ step }) => {
    ...
    await step.run("fan-out", async () => {
            // One send call with all events. Vercel Workflow batches on the wire.
      await workflow.send(
        orgs.map((o) => ({
