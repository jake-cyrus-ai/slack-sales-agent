/**
 * Send Organization Invite - Admin Vercel Workflow Function
 *
...
export const sendOrganizationInvite = workflow.createFunction(
  {
  ...
  async ({ event, step }) => {
    ...
    const result = await step.run("notify-inviter", async () => {
      ...
      // Fallback: log so the URL is visible in server logs / Vercel Workflow trace
