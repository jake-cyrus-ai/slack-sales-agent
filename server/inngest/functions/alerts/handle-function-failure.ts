/**
 * Global Inngest Failure Handler
 *
 * Listens for the `inngest/function.failed` system event to provide centralized
 * alerting when any Inngest function fails after exhausting all retries.
 *
 * Alerts are sent to:
    ...
    blocks: [
      ...
      },
      {
        ...
        elements: [
          {
            type: "mrkdwn",
            text: `<https://app.inngest.com/env/production/functions/${encodeURIComponent(functionId)}/logs/${runId}|View in Inngest Dashboard>`,
          },
        ],
      },
      {
...
const formatErrorForPagerDuty = (
  ...
  return {
    ...
    event_action: "trigger",
    dedup_key: `inngest-failure-${functionId}-${runId}`,
    payload: {
      summary: `Inngest function "${functionId}" failed: ${error.message.slice(0, 200)}`,
      severity: "error",
      source: "inngest",
      timestamp,
      custom_details: {
    ...
    links: [
      {
        href: `https://app.inngest.com/env/production/functions/${encodeURIComponent(functionId)}/logs/${runId}`,
        text: "View in Inngest Dashboard",
      },
    ],
  };
};
