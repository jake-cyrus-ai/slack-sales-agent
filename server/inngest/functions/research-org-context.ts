/**
 * Research Org Context — Inngest Function
 *
...
export const researchOrgContext = inngest.createFunction(
  {
  ...
  async ({ event, step }) => {
    ...
    // Step 4b: Browserbase deep browse — visit org website + competitor homepages for live data
    const browseResults = await step.run("browserbase-research", async () => {
      if (!process.env.BROWSERBASE_API_KEY || !process.env.BROWSERBASE_PROJECT_ID) {
        return { orgPage: null, competitorPages: [] };
      }
    ...
    const synthesized = await step.run("synthesize-context", async () => {
      ...
      // Supplement Exa competitor text with live Browserbase extractions
