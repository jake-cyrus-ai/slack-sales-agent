/**
 * Document Share Handler for Inngest
 *
 * Detects document share requests and uploads actual files to Slack.
 * Ported from server/src/slack/document-share.ts, adapted for Inngest context
 ...
 * Uses raw fetch for Slack API calls (no @slack/web-api dependency in Inngest).
 */
export async function shareDocumentInInngest(
  botToken: string,
  channel: string,
