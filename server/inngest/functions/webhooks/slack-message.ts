...
import { detectDocumentShareRequest, shareDocumentInInngest } from "./document-share";
...
// ─── Types for step.run return values (Inngest returns unknown) ─────────────
 ...
 * so Inngest can still retry the function. If a later retry succeeds, the
  ...
  try {
    ...
    if (backup.success && backup.response) {
      ...
      await writeErrorEvent({
        ...
        inngestRunId: params.runId,
        inngestFunctionId: "handle-slack-message",
 ...
 * Bump this when the Inngest plan supports higher concurrency.
 *
 * IDEMPOTENCY:
 * ────────────
 ...
 * ──────────────────────────────────
 * Layer 1: Inngest concurrency limit (per-thread serialization)
 * Layer 2: Inngest idempotency key (prevents re-execution of same event)
 ...
 */
export const handleSlackMessage = inngest.createFunction(
  {
  ...
  async ({ event, step, runId, attempt }) => {
    ...
    // classification — all DB reads + regex. Saves ~5 Inngest step round-trips
    ...
    if (ctx.docShare.isShareRequest && ctx.docShare.query && ctx.organizationId) {
      const shareResult = await step.run("document-share", async () => {
        ...
        const result = await shareDocumentInInngest(
    ...
    if (classification.intent === "search-docs") {
      ...
      // ~2-3s overhead of a second Inngest function hop + duplicate
      ...
      // telemetry row and rethrows so Inngest can retry.
      try {
      ...
      // The legacy chat/save-memory Inngest event has been removed to avoid double-saving.

      // Success — mark any earlier unresolved error rows for this Inngest
      ...
      return { status: "ok", intent: "chat" };
        ...
        // then rethrow so Inngest can retry the function.
